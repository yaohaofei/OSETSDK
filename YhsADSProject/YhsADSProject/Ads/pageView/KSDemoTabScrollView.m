...
@interface KSDemoTabScrollView ()<UIScrollViewDelegate>

@end

@implementation KSDemoTabScrollView
...
-(void)configParameter:(DirectionStyle)directionStyle
               viewArr:(NSArray<UIView*>*)viewArr
              tabWidth:(NSInteger)tabWidth
             tabHeight:(NSInteger)tabHeight
                 index:(NSInteger)index
                 block:(TabClickBlock) clickBlock {
    _viewArr = viewArr;
...
//    _tagIndex = index;
    ...
    } else {//垂直滚动,基本和水平滚动差不多,只是将tab宽高对换
        //获取scrollview宽度
        NSInteger maxWidth = self.frame.size.height;
        //获取当前点击的tab所处的位置大小
...
-(void)updateTagLine:(NSInteger)index {
    ...
    if(_direction == horizontal) {
        ...
        [UIView animateWithDuration:0.1 animations:^ {
            self.tagLine.frame = CGRectMake(index*self.tabWidth, self.tabHeight-tagLineheight-0.5, self.tabWidth, tagLineheight);
        ...
        //标记线切换动画
        [UIView animateWithDuration:0.1 animations:^ {
            self.tagLine.frame = CGRectMake(self.tabWidth-tagLineheight-0.5, self.tabHeight*index, tagLineheight, self.tabHeight);
        }  completion:^(BOOL finished)  {
            self.tagIndex = index;
